...
HAL_StatusTypeDef BatGauge_Init(void)
{
    ...
    if (dev_type != BQ27441_DEVICE_TYPE_ID) return HAL_ERROR;

    st = read16(BQ27441_REG_FLAGS, &flags);
    if (st != HAL_OK) return st;

    if (flags & BQ27441_FLAG_ITPOR) {
        st = BatGauge_Configure(true);
        if (st != HAL_OK) return st;
    }

    BatGauge_Control(BQ27441_CTRL_CLEAR_HIBERNATE, NULL);
...
HAL_StatusTypeDef BatGauge_ReadAll(BatGauge_Data_t *data)
{
    ...
    st = BatGauge_Control(BQ27441_CTRL_STATUS, &data->ctrl_status);
    return st;
}
...
HAL_StatusTypeDef BatGauge_Configure(bool itpor)
{
    ...
    /* 10. Post-config housekeeping */
    BatGauge_Control(BQ27441_CTRL_CLEAR_HIBERNATE, NULL);
    HAL_Delay(10);
    BatGauge_Control(BQ27441_CTRL_IT_ENABLE, NULL);
    HAL_Delay(10);

    /* 11. If ITPOR was active, announce battery insertion and reset */
    if (itpor) {
        BatGauge_Control(BQ27441_CTRL_BAT_INSERT, NULL);
        HAL_Delay(10);
        BatGauge_SoftReset();
        HAL_Delay(500);
    }
